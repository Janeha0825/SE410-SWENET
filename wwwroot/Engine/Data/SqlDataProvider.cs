    ...
    public class SqlDataProvider : IDataProviderBase {
		...
		****************************************************************/
		public void AddRating( Rating rating ) {
			...
			cmd.Connection = new SqlConnection( Globals.DatabaseConnectionString );
			cmd.CommandText = "INSERT INTO Ratings (PostID, Rating) " +
				"VALUES (@PostID, @Rating) ";
			cmd.Parameters.Add( new SqlParameter( "@PostID", rating.PostID ) );
			cmd.Parameters.Add( new SqlParameter( "@Rating", rating.Value ) );
		...
		//
		/// <summary>
		/// Gets a rating for a given post.
		/// </summary>
		//
		****************************************************************/
		public Rating GetRating(int postID) {
			Rating retVal = new Rating();
			retVal.PostID = postID;

			IDbCommand cmd = new SqlCommand();
			cmd.Connection = new SqlConnection( Globals.DatabaseConnectionString );
			cmd.CommandText = "SELECT Rating FROM Ratings WHERE PostID = @PostID";
			...
			try {
				...
				if ( reader.Read() ) {
					retVal.Value = Convert.ToInt32( reader["Rating"] );
				}
			} catch ( SqlException e ) {
				throw e;
			} finally {
		...
		//
		/// <summary>
		/// Gets a rating for a given user and parent post ID.
		/// </summary>
		/// <returns>
		/// The user's rating for the given parent ID or null if there
		/// is none.
		/// </returns>
		//
		****************************************************************/
		public Rating GetRatingForUser( string username, int parentID ) {
			Rating retVal = new Rating();

			IDbCommand cmd = new SqlCommand();
			cmd.Connection = new SqlConnection( Globals.DatabaseConnectionString );
			cmd.CommandText = "SELECT Ratings.Rating, Ratings.PostID FROM Ratings " +
				"INNER JOIN Posts ON Ratings.PostID = Posts.PostID " +
				"WHERE (Posts.ParentID = @ParentID) AND (Posts.UserName = @UserName) " +
				"AND (Posts.PostType = 2)";
			...
			try {
				...
				// If there is a rating, record the rating and post ID.
				// Otherwise, set the return value to null, indicating
				// that there was not rating for the given user.
				if ( reader.Read() ) {
					retVal.Value = Convert.ToInt32( reader["Rating"] );
					retVal.PostID = Convert.ToInt32( reader["PostID"] );
				} else {
					retVal = null;
				}
			} catch ( SqlException e ) {
        ...
        *****************************************************************/
        ...
        public Post AddPost(Post postToAdd, string username) {
			...
			SqlParameter parameterPostType = new SqlParameter( "@PostType", SqlDbType.Int, 4 );
			parameterPostType.Value = (int)postToAdd.PostType;
			myCommand.Parameters.Add( parameterPostType );
